...
async def analyze_news():
    ...
    start_date = data.get('start_date', "2025-01-01")  # 用户传递的开始日期，默认为 "2025-01-01"
    end_date = data.get('end_date', "2025-01-06")  # 用户传递的结束日期，默认为 "2025-01-06"
    num_records = data.get('num_records', 5)  # 用户传递的新闻数量，默认为 20
    if not keyword:
        return jsonify({"success": False, "message": "行业标签不能为空"}), 400

    result = await NewsService.fetch_and_analyze_news(keyword, start_date, end_date, num_records)
    return jsonify(result)
